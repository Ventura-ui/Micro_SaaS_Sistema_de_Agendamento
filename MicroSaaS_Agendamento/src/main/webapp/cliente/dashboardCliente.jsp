...
<body>
	...
	<div class="container mt-5 shadow-lg rounded">
		...
		<div class="d-grid gap-2 col-md-8 mx-auto">
			<a href="/MicroSaaS_Agendamento/ListarAgendaCliente"
				class="btn btn-primary">Meus agendamentos</a> <a
				href="/MicroSaaS_Agendamento/LogoutServlet" class="btn btn-danger">Fazer
				Logout</a>
		</div>
		...
		<form method="get" action="/MicroSaaS_Agendamento/dashboardCliente">
			<div class="mb-3">
				<label for="cidade" class="form-label">Filtrar por cidade:</label> <select
					name="cidade" id="cidade" class="form-select">
					<option value="">Todas</option>
					<option value="Araraquara"
						${cidadeSelecionada == 'Araraquara' ? 'selected' : ''}>Araraquara</option>
					<option value="São Carlos"
						${cidadeSelecionada == 'São Carlos' ? 'selected' : ''}>São
						Carlos</option>
					<option value="Matão"
						${cidadeSelecionada == 'Matão' ? 'selected' : ''}>Matão</option>
				</select>
			</div>
											...
											<div class="carousel-inner">
												<c:forEach var="img" items="${p.imagens}" varStatus="status">
													<div
														...
														<img
															src="/MicroSaaS_Agendamento/imagensPrestadores/${img}"
		...
		<p class="text-center mt-4">
			<c:choose>
				<c:when test="${pagina > 0}">
					<a class="btn btn-secondary me-2 btn-pagination"
			...
			<c:choose>
				<c:when test="${pagina < totalPaginas - 1}">
					<a class="btn btn-secondary btn-pagination"
